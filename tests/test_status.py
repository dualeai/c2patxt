...
def test_the_exception_hierarchy_uses_the_same_vocabulary() -> None:
    """MarkCorruptError.code must BE a StatusCode, not a look-alike string.

    It was a bare literal, so the exception channel and the Verdict channel could
    drift apart silently. A caught exception must map onto the same vocabulary a
    Verdict reports.
    """
    from c2patxt.exceptions import MarkCorruptError

    error = MarkCorruptError("boom", "abc", 1)
    assert error.code is StatusCode.TEXT_CORRUPTED_WRAPPER
    assert str(error.code) == "manifest.text.corruptedWrapper"


def test_every_status_code_explains_itself() -> None:
    """These are WIRE STRINGS an integrator interpolates into their own API responses,
    and all 31 shipped with no explanation: three had no sentence anywhere in the
    repository, thirteen appeared in no markdown, five had their best mention in a `#`
    comment, and only four told a reader what to DO.

    ASSIGNED, NOT WRITTEN AS LITERALS, and that is forced rather than chosen: an enum
    discards the string after ``NAME = "value"``, so ``StatusCode.NAME.__doc__`` returns
    the CLASS docstring. The form a reader would expect is the one form that does not
    work.

    A NEW CODE CANNOT ARRIVE BARE, which is the point of this test: `_EXPLANATIONS` is
    keyed by member, so adding a code without one fails here rather than shipping a
    wire string nobody can interpret.
    """
    # The mapping IS the mechanism being tested: it is what makes a bare new code
    # impossible, so the test has to reach for it.
    from c2patxt.status import (
        _EXPLANATIONS,  # pyright: ignore[reportPrivateUsage] -- the completeness of this mapping is the property
    )

    assert set(_EXPLANATIONS) == set(StatusCode), (
        f"codes with no explanation: {sorted(c.name for c in StatusCode if c not in _EXPLANATIONS)}"
    )

    for code in StatusCode:
        text = code.__doc__
        assert text is not None and text is not StatusCode.__doc__, f"{code.name} has no docstring of its own"
        assert len(text.split()) >= 15, f"{code.name}'s explanation is too short to say condition and clause"

    # Distinct: a copied-and-unedited entry is the likely way this rots.
    texts = [code.__doc__ for code in StatusCode]
    assert len(set(texts)) == len(texts), "two codes share an explanation"


def test_the_success_codes_say_so_and_untrusted_says_it_does_not_invalidate() -> None:
    """Three SUCCESS codes read like failures to someone skimming, and one FAILURE code
    is not a failure at all.

    ``signingCredential.untrusted`` sits in the FAILURE bucket because the specification
    puts it there, but 14.3.5 defines a *Valid* manifest without requiring trust. A
    correct self-signed mark verifies VALID carrying it. CLAUDE.md calls treating that
    as an error "the single most likely mistake in this repository", so the code's own
    text has to say it.
    """
    for code in (
        ...
        assert (code.__doc__ or "").startswith("SUCCESS"), f"{code.name} is a success code and must open by saying so"

    untrusted = StatusCode.SIGNING_CREDENTIAL_UNTRUSTED.__doc__ or ""
    assert "DOES NOT INVALIDATE" in untrusted
    assert "14.3.5" in untrusted, "the clause that makes this not an error must be cited"
