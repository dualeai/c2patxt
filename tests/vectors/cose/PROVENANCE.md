...
`eddsa-01` and `eddsa-sig-01` use **Ed25519 with `alg` = -8**, the only EdDSA
instance C2PA 2.4 §13.2.1 permits. `eddsa-sig-02` is **Ed448**, which that clause
forbids; `tests/test_cose.py` holds both cases.

`sign-pass-*` and `sign-fail-*` — upstream's `sign1-tests/`, flattened here — add three
passing and six **failing** cases, all **ES256 over P-256**. They carry attached
payloads and no C2PA `x5chain`, so this suite uses them as `Sig_structure` oracles.
`sign-fail-05` does not exist upstream.
...
(§13.2.2), generator-side protected `x5chain` plus validator compatibility for either
bucket (§14.5), and exactly one identity credential across both buckets (§14.2).
